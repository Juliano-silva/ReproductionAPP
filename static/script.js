    ...
    fetch("/DadosMusic").then(function (response) {
        response.json().then(async (data) => {
            ...
            for (var i = 0; i < MinhasMusicas.Name_Music.length; i++) {
                ...
                TPImage.src = ImagesPrincipal.src = Image.src = `http://img.youtube.com/vi/${ImageRenovado}/mqdefault.jpg`
                }
                else if(
                    TPImage.src == "http://img.youtube.com/vi/undefined/mqdefault.jpg" ||
                    ...
                    if (document.getElementById(this.id).checked == true) {
                        ...
                        function Frente() {
                            ...
                            if (MusicIDs < MinhasMusicas.Name_Music.length) {
                                MusicIDs++
                                if(localStorage.Musicas){
                                    var ImageRenovado =  JSON.parse(BuscarImageLocal)[MusicIDs]?.replace("https://www.youtube.com/watch?v=", "")?.replace("https://music.youtube.com/watch?v=", "")?.slice(0, 11)
                                ...
                                if(localStorage.Musicas){
                                    var ImageRenovado =  JSON.parse(BuscarImageLocal)[0]?.replace("https://www.youtube.com/watch?v=", "")?.replace("https://music.youtube.com/watch?v=", "")?.slice(0, 11)
                                    TPImage.src = ImagesPrincipal.src = `http://img.youtube.com/vi/${ImageRenovado}/mqdefault.jpg`
                                }
                                document.querySelectorAll("#Playlist_List > li").forEach(el => el.id = `${MusicIDs}`)
                            }
